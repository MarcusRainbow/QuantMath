...
impl FixingTable {
    ...
    pub fn get(&self, id: &str, date_time: DateTime) -> Result<Option<f64>, qm::Error> {
        match self.get_optional(id, date_time) {
            Some(fixing) => Ok(Some(fixing)),

            None => {
                if date_time.date() < self.fixings_known_until {
                    Err(missing_fixing(id, date_time))
                } else {
                    Ok(None)
                }
            }
...
impl Fixings {
    ...
    pub fn new(id: &str, fixings: &[(DateTime, f64)]) -> Result<Fixings, qm::Error> {
...
fn duplicate_fixing(id: &str, v1: f64, v2: f64, date_time: DateTime) -> qm::Error {
    qm::Error::new(&format!(
        "Duplicate fixing for \"{}\": \
        {} and {} both present at {}",
        id, v1, v2, date_time
    ))
}
...
mod tests {
            ...
            &[
                ...
                (
                    ...
                    &[
                        ...
                        (DateTime::new(today - 7, TimeOfDay::Open), 223.2),
                    ],
                ),
            ],
        )
    ...
    fn find_fixing_today() {
        ...
        let fixing = fixings
            .get("BT.L", DateTime::new(today, TimeOfDay::Open))
            .unwrap();
    ...
    fn missing_fixing_past() {
        ...
        let today = fixings.fixings_known_until();
        let _ = fixings
            .get("BT.L", DateTime::new(today - 9, TimeOfDay::Close))
            .unwrap();
    }
    ...
    fn find_fixing_past() {
        ...
        let today = fixings.fixings_known_until();
        let fixing = fixings
            .get("BT.L", DateTime::new(today - 7, TimeOfDay::Open))
            .unwrap();
        if let Some(f) = fixing {
            assert_eq!(f, 123.2);
        } else {
    ...
    fn find_optional_fixing_past() {
        let fixings = sample_fixings();
        let today = fixings.fixings_known_until();
        let fixing = fixings.get_optional("BT.L", DateTime::new(today - 9, TimeOfDay::Open));
        if let Some(f) = fixing {
            assert_eq!(f, 123.1);
        } else {
