...
impl sd::Serialize for RcCurrency {
    ...
    where
        S: sd::Serializer,
    {
        self.serialize_with_dedup(serializer, &DEDUP_CURRENCY, |s| self.deref().serialize(s))
    }
}
...
impl<'de> sd::Deserialize<'de> for RcCurrency {
    ...
    {
        Self::deserialize_with_dedup(deserializer, &DEDUP_CURRENCY, |d| {
            string_or_struct::<Currency, Arc<Currency>, D>(d)
        })
    }
}
...
impl Equity {
    pub fn new(id: &str, credit_id: &str, currency: RcCurrency, settlement: RcDateRule) -> Equity {
        Equity {
            id: id.to_string(),
