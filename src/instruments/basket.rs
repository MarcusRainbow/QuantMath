...
impl Instrument for Basket {
    ...
    fn time_to_day_fraction(&self, date_time: DateTime) -> Result<DateDayFraction, qm::Error> {
    ...
    fn fix(
        ...
        match fix_all(&self.basket, fixing_table)? {
            Some(basket) => {
                ...
                let replacement: RcInstrument = RcInstrument::new(Qrc::new(Arc::new(Basket::new(
                    &id,
                    self.credit_id(),
                    self.currency.clone(),
...
impl Priceable for Basket {
    ...
    fn prices(
        ...
        for &(weight, ref underlying) in self.basket.iter() {
            let priceable = underlying
                .as_priceable()
                .ok_or_else(|| qm::Error::new("The underlying of an basket must be priceable"))?;
...
pub mod tests {
    ...
    use crate::data::curves::ZeroRateCurve;
    ...
    impl PricingContext for SamplePricingContext {
        ...
        fn forward_curve(
            ...
            let divs = DividendStream::new(
                &Vec::new(),
                RcRateCurve::new(Arc::new(ZeroRateCurve::new(base_date))),
            );
    ...
    fn test_basket() {
        ...
        let price = basket
            .price(&context, DateTime::new(date, TimeOfDay::Close))
            .unwrap();
        assert_approx(price, 120.0 * 0.4 + 230.0 * 0.6);

        let forward = basket
            .price(&context, DateTime::new(date + 365, TimeOfDay::Close))
            .unwrap();
